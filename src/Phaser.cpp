...
PhaserWidget::PhaserWidget(PhaserWidget::M *model) : ModuleWidget()
{
    ...
    addInput(createInput<PJ301MPort>(Vec(bg->cx() - 40, RACK_HEIGHT - 120), module, M::SIGNAL_IN));
    addOutput(createOutput<PJ301MPort>(Vec(bg->cx() + 40, RACK_HEIGHT - 120), module, M::PHASED));
}
