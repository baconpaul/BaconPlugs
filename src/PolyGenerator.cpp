...
PolyGeneratorWidget::PolyGeneratorWidget(PolyGenerator *model)
    ...
    {
        if( i == 1 || i == 2 )
            addParam(createParam<RoundBlackSnapKnob>(Vec(7, y0 + dy * i + 16), module, M::BPM_PARAM + i));
        else
            addParam(createParam<RoundBlackKnob>(Vec(7, y0 + dy * i + 16), module, M::BPM_PARAM + i));
        if( i < 2 ) {
            addChild(MultiDigitSevenSegmentLight<BlueLight, 2, 3>::create(
                         Vec(box.size.x - 7 - 3 * 14, y0 + dy * i + 16 + 4), module,
                         M::BPM_LIGHT + i));
        }
        if( i == 2 ) {
            addChild(DotMatrixLightTextWidget::create(
                         ...
                         module, 8,
                         PolyGenerator::getPatternNameDirty,
                         PolyGenerator::getPatternName));
        ...
        if( i == 3 ) {
            addChild(DotMatrixLightTextWidget::create(
                         Vec(41, y0 + dy * i + 16 + diffY2c<RoundBlackSnapKnob, DotMatrixLightTextWidget>()),
                         module, 8,
                         PolyGenerator::getExtraLabelDirty,
                         PolyGenerator::getExtraLabel));

        }
        if( i == 3 )
        {
            addChild(createLight<SmallLight<BlueLight>>(
                         Vec(40, y0 + dy * i), module, M::EXTRA_LIGHT));
