...
ALingADingWidget::ALingADingWidget(ALingADing *module)
{
    ...
    addInput(createInput<PJ301MPort>(Vec(7, 70), module, ALingADing::SIGNAL_INPUT));
    ...
    bg->addLabel(Vec(bg->cx() + 10, 140 + 72), "Wet", 13, NVG_ALIGN_LEFT | NVG_ALIGN_TOP);
    bg->addLabel(Vec(bg->cx() - 10, 140 + 72), "Dry", 13, NVG_ALIGN_RIGHT | NVG_ALIGN_TOP);

    addParam(
        createParam<RoundHugeBlackKnob>(Vec(bg->cx(56), 150), module, ALingADing::WET_DRY_MIX));

    Vec outP = Vec(bg->cx(24), RACK_HEIGHT - 15 - 43);
    bg->addPlugLabel(outP, BaconBackground::SIG_OUT, "out");
    addOutput(createOutput<PJ301MPort>(outP, module, ALingADing::MODULATED_OUTPUT));
}

Model *modelALingADing = createModel<ALingADing, ALingADingWidget>("ALingADing");
