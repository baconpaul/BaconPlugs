...
struct SortaChorus : virtual Module {
  ...
  public:
    enum ParamIds { DEPTH, SPEED, SHAPE, NOISE, NUM_PARAMS };

    enum InputIds { SIGNAL_INPUT, NUM_INPUTS };

    enum OutputIds { CHORUSED_OUTPUT, NUM_OUTPUTS };
    ...
    void process(const ProcessArgs &args) override {
        ...
        outputs[CHORUSED_OUTPUT].setVoltage(
            (inputs[SIGNAL_INPUT].getVoltage() + depth * chSig) /
            (1.0 + depth));
    }
};
...
SortaChorusWidget::SortaChorusWidget(SortaChorus *module) : ModuleWidget() {
    ...
    addOutput(
        createOutput<PJ301MPort>(outP, module, SortaChorus::CHORUSED_OUTPUT));
}

Model *modelSortaChorus = createModel<SortaChorus, SortaChorusWidget>(
    "Bacon Music", "SortaChorus", "SortaChorus", CHORUS_TAG);
