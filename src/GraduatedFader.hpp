...
template <int H> struct GraduatedFader : app::SliderKnob {
    ...
    GraduatedFader() {
        ...
        handle = new BufferedDrawFunctionWidget<GraduatedFader<H>>(
            Vec(0,0), Vec(20,41), this, &GraduatedFader<H>::drawHandle);
        fb->addChild(handle);
    ...
    void drawHandle(NVGcontext *vg) {
        ...
        NVGpaint vgr = nvgLinearGradient(vg, 0, 0, 0, 19, nvgRGBA(170, 170, 190, 255), nvgRGBA( 220, 200, 240, 255) );
        nvgFillPaint(vg, vgr);
        nvgFill(vg);

        nvgBeginPath(vg);
        nvgRoundedRect(vg,0,19,20,21,3);
        vgr = nvgLinearGradient(vg, 0, 41, 0, 21, nvgRGBA(170, 170, 190, 255), nvgRGBA( 220, 200, 240, 255) );
        nvgFillPaint(vg, vgr);
        nvgFill(vg);
