...
GlissinatorWidget::GlissinatorWidget(Glissinator<Module> *model)
    : ModuleWidget() {
    setModule(model);
    ...
    ParamWidget *slider = createParam<GraduatedFader<230>>(
        Vec(bg->cx(29), 23), module, G::GLISS_TIME);
    ...
    bg->addLabel(Vec(10, RACK_HEIGHT - 102), "gliss", 11,
                 NVG_ALIGN_LEFT | NVG_ALIGN_BOTTOM,
                 componentlibrary::SCHEME_WHITE);
    bg->addLabel(Vec(10, RACK_HEIGHT - 90), "gate", 11,
                 NVG_ALIGN_LEFT | NVG_ALIGN_BOTTOM,
                 componentlibrary::SCHEME_WHITE);
    addChild(createLight<SmallLight<BlueLight>>(
        Vec(bg->cx() - 4, RACK_HEIGHT - 120 + 38 / 2 - 3), module,
        G::SLIDING_LIGHT));
    addOutput(createOutput<PJ301MPort>(Vec(bg->cx() + 5, RACK_HEIGHT - 114),
                                       module, G::GLISSING_GATE));
}

Model *modelGlissinator =
    createModel<Glissinator<Module>, GlissinatorWidget>("Glissinator");
